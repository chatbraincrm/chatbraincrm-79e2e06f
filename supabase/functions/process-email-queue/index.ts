...
// Erro com status HTTP para replicar comportamento anterior
...
async function sendViaResend(payload: any, apiKey: string): Promise<void> {
  ...
  if (!resp.ok) {
    const body = await resp.json().catch(() => ({ message: 'erro desconhecido' }))
    const retryAfter = resp.status === 429
      ? parseInt(resp.headers.get('Retry-After') ?? '60', 10)
      : undefined
    throw new SendError(
...
function getRetryAfterSeconds(error: unknown): number {
  if (error instanceof SendError && error.retryAfterSeconds != null) {
